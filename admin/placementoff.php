...
$query="select * from users where roles='placementofficer'";
  ...
  <section id="minimal-statistics">
    <div class="row">
      <div class="col-12 mt-3 mb-1">
        <h4 class="text-uppercase mt-3" style="color:#4649FF;font-family: 'Noto Serif Ahom', serif;font-size:45px;">ADD Placement Officer Details</h4>
      </div>
    </div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form action="partials/db_action.php" method="POST" enctype="multipart/form-data">
        <label for="fileInput">
          <i class="fa-solid fa-cloud-arrow-down"style="font-size: 89px;"></i>
          <input id="fileInput" type="file" name="import_file" style="display: none;">
        </label>
      <div class="form-group d-flex justify-content-center mt-0">
        <h4>Select a file</h4>
      </div>
      <div class="form-group d-flex justify-content-center">
        <button type="submit" class="btn btn-info" name="save_excel_data">UPLOAD</button>
      </div>
    </form>
    </div>
    </div>
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Placement-Officers</h5>
      ...
      <div class="modal-body">
        <form action="partials/db_action.php" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control" value="placementofficer" name="role" id="exampleInputEmail1" aria-describedby="emailHelp" readonly>
          </div>
          <div class="modal-footer">
            <button type="submit" class="btn btn-secondary"  data-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary" name="addplacementofficer">Submit</button>
          </div>
        </form>
      </div>
    </div>
...
<tr>
  ...
  $query_run= mysqli_query($connect,$query);
   ...
   while($row2=mysqli_fetch_assoc($query_run))
            {
			$output =$row2['departmentname'];
			if($row2['departmentname']>0)
			{
			?>
  ...
  <td>
    <div class="d-flex">
      <form method="GET" action=''>
        <input type='hidden' name='id' value='<?php echo $row['id'] ?>'/><a href="placementoff.php?id=<?php echo $row['id'] ?>" name='place_edit' class="px-3 btn btn-primary btn-sm">Edit</a>
      </form>
      <form method="POST" action='partials/db_action.php'>
        <input type='hidden' name='id' value='<?php echo $row['id'] ?>'/>
        <?php
        echo "<input  class='btn btn-danger ml-2 btn-sm' type='submit' style='outline:none;'  onclick='return Delete_placementoff();' name='place_delete' value='Delete' />";?>
        </form>
      </div>   
  </td>
</tr>
...
<!-- DELETE FUNCTION -->
<script>
function Delete_placementoff(){
  return confirm('Are You Sure You Want to Delete this Placementofficer');
}
</script>
...
<div class="modal fade" id="editexampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Edit Placement-Officers Details</h5>
        <button type="submit" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        ...
        <input type="hidden" class="form-control" name="e_id" id="exampleInputEmail1" aria-describedby="emailHelp" value="'.$edit['id'].'">
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-secondary"  data-dismiss="modal">Close</button>
            <button type="submit" class="btn btn-primary" name="update_placementoff">Update</button>
          </div>
        </form>
      </div>
    </div>
